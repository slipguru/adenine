...
From an algorithmic standpoint, \ade is built upon the concept of \emph{pipeline}, that is a sequence of four fundamental steps:
\begin{enumerate*}[label=(\roman*)]
  ...
  \item clustering
\end{enumerate*}.
For each step, a fair number of off-the-shelf algorithms implementations are available (see Table~\ref{sec:implem}). None of such steps are mandatory. The vast majority of the algorithm implementation is inherited from the machine learning \texttt{Python} library \texttt{scikit-learn} \citep{scikit-learn, sklearn_api}. \todo{except KNN imputing}.
